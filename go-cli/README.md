# longevity-competition-cli

boru-driven command-line client **and** interactive REPL for the LongevityCompetition
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/longevity-competition-cli)
make build

# 2. See usage (words, entities, env vars)
./longevity-competition-cli --help

# 3. Provide credentials once, via the environment
export LONGEVITY_COMPETITION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./longevity-competition-cli list athlete
./longevity-competition-cli list bortz_age

# 5. Override the API base URL for a single call
LONGEVITY_COMPETITION_BASE=https://api.example.com ./longevity-competition-cli list athlete

# 6. No arguments -> interactive REPL
./longevity-competition-cli
longevity-competition> list athlete
longevity-competition> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/longevity-competition-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/longevity-competition-cli list athlete
   ```
...
Run with no arguments to open a REPL (prompt `longevity-competition>`). Each line is
evaluated as its own boru expression:

```text
$ ./longevity-competition-cli
longevity-competition> list athlete
longevity-competition> /help
longevity-competition> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `athlete`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/longevity-competition-cli`. |
...
(`./longevity-competition-cli <expr>`) and interactively (the REPL), and expressions compose the
