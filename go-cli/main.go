// Command LongevityCompetition-cli is a CLI + REPL for the LongevityCompetition SDK,
...
const prompt = "longevity-competition"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "athlete bortz_age competition leaderboard pheno_age rank_preview reference"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LONGEVITY_COMPETITION_APIKEY (api key), LONGEVITY_COMPETITION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LONGEVITY_COMPETITION_APIKEY carries the API key and
	// LONGEVITY_COMPETITION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LONGEVITY_COMPETITION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LONGEVITY_COMPETITION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLongevityCompetitionSDK(opts)
