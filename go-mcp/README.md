# longevity-competition-mcp

[MCP](https://modelcontextprotocol.io) server exposing the LongevityCompetition SDK as
two agent tools — `longevity-competition_list` and `longevity-competition_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/longevity-competition-mcp)
make build

# 2. Provide credentials via the environment
export LONGEVITY_COMPETITION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user longevity-competition \
  -- /absolute/path/to/longevity-competition-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./longevity-competition-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// longevity-competition_list: first page of records
{ "entity": "athlete" }
{ "entity": "athlete", "query": { } }

// longevity-competition_load: one record by id
{ "entity": "athlete", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/longevity-competition-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user longevity-competition \
     -- "$PWD"/dist/*/longevity-competition-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `longevity-competition_list` and `longevity-competition_load` tools now appear
   in new sessions. Ask the agent to *"list athlete using longevity-competition"*
   and it calls `longevity-competition_list` with `{"entity":"athlete"}`.
...
```sh
export LONGEVITY_COMPETITION_APIKEY=sk_live_xxx            # API key
export LONGEVITY_COMPETITION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `longevity-competition_list` tool
...
### Call the `longevity-competition_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `longevity-competition_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `longevity-competition_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LONGEVITY_COMPETITION_APIKEY` | API key sent with every request. |
| `LONGEVITY_COMPETITION_BASE` | Optional override of the API base URL. |

### Entities

The 7 entities valid as the `entity` argument:

athlete | bortz_age | competition | leaderboard | pheno_age | rank_preview | reference

### Smoke test via HTTP (raw JSON-RPC)

```sh
./longevity-competition-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"longevity-competition_load","arguments":{"entity":"athlete","query":{"id":1}}}}'
```
