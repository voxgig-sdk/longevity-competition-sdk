# LongevityCompetition SDK test runner
...
class LongevityCompetitionTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            LongevityCompetitionTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = LongevityCompetitionTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = LongevityCompetitionTestRunner.getenv("LONGEVITY_COMPETITION_TEST_LIVE")
        override = LongevityCompetitionTestRunner.getenv("LONGEVITY_COMPETITION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = LongevityCompetitionTestRunner.getenv(key)
        ...
        explain = LongevityCompetitionTestRunner.getenv("LONGEVITY_COMPETITION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if LongevityCompetitionTestRunner._test_control is not None:
            return LongevityCompetitionTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                LongevityCompetitionTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            LongevityCompetitionTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = LongevityCompetitionTestRunner.load_test_control()
...
def load_env_local():
    LongevityCompetitionTestRunner.load_env_local()


def env_override(m):
    return LongevityCompetitionTestRunner.env_override(m)


def entity_data(v):
    return LongevityCompetitionTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return LongevityCompetitionTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return LongevityCompetitionTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return LongevityCompetitionTestRunner.load_test_control()


def live_client_options():
    return LongevityCompetitionTestRunner.live_client_options()


def live_delay_ms():
    return LongevityCompetitionTestRunner.live_delay_ms()
