...
class TestLeaderboardDirect:

    def test_should_direct_list_leaderboard(self):
        setup = _leaderboard_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-leaderboard", "live" if setup["live"] else "unit")
        if _skip:
...
def _leaderboard_direct_setup(mockres):
