    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set LONGEVITY_COMPETITION_TEST_PHENO_AGE_ENTID JSON to run live")
        client = setup["client"]
        ...
        pheno_age_ref01_data = helpers.to_map(runner.entity_data(pheno_age_ref01_ent.create(pheno_age_ref01_data, None)))
...
def _pheno_age_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "LONGEVITY_COMPETITION_TEST_PHENO_AGE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("LONGEVITY_COMPETITION_TEST_PHENO_AGE_ENTID"))
