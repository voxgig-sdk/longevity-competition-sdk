...
class TestReferenceDirect:

    def test_should_direct_list_reference(self):
        setup = _reference_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-reference", "live" if setup["live"] else "unit")
        if _skip:
...
def _reference_direct_setup(mockres):
    ...
    if live:
        ...
        client = LongevityCompetitionSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
