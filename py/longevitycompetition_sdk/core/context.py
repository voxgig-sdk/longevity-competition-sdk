...
from longevitycompetition_sdk.core.helpers import get_ctx_prop, to_map
...
class LongevityCompetitionContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = LongevityCompetitionControl()
        ...
        if isinstance(sp, LongevityCompetitionSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, LongevityCompetitionResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, LongevityCompetitionResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return LongevityCompetitionOperation({})
        ...
        op = LongevityCompetitionOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return LongevityCompetitionError(code, msg, self)
