...
local PhenoAgeEntity = {}
PhenoAgeEntity.__index = PhenoAgeEntity


function PhenoAgeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PhenoAgeEntity)
  self._name = "pheno_age"
  self._client = client
...
function PhenoAgeEntity:get_name()
  return self._name
end


function PhenoAgeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PhenoAgeEntity.new(self._client, opts)
end
...
function PhenoAgeEntity:mark_deleted()
  self._deleted = true
end


function PhenoAgeEntity:deleted()
  return true == self._deleted
end


function PhenoAgeEntity:data_set(args)
  if args ~= nil then
...
function PhenoAgeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PhenoAgeEntity:match_set(args)
  if args ~= nil then
...
function PhenoAgeEntity:match_get()
...
function PhenoAgeEntity:stream(action, args, callopts)
...
---@param reqdata PhenoAgeCreateData
---@param ctrl? table
---@return PhenoAge
---@return string? err
function PhenoAgeEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function PhenoAgeEntity:_run_op(ctx, post_done)
  local utility = self._utility
