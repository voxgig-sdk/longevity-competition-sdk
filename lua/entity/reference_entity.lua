...
ReferenceEntity.__index = ReferenceEntity


function ReferenceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReferenceEntity)
...
function ReferenceEntity:get_name()
  return self._name
end


function ReferenceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReferenceEntity.new(self._client, opts)
end
...
function ReferenceEntity:mark_deleted()
  self._deleted = true
end


function ReferenceEntity:deleted()
  return true == self._deleted
end


function ReferenceEntity:data_set(args)
  if args ~= nil then
...
function ReferenceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReferenceEntity:match_set(args)
  if args ~= nil then
...
function ReferenceEntity:match_get()
...
function ReferenceEntity:stream(action, args, callopts)
...
---@param reqmatch ReferenceListMatch
---@param ctrl? table
---@return Reference[]
---@return string? err
function ReferenceEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ReferenceEntity:_run_op(ctx, post_done)
  local utility = self._utility
