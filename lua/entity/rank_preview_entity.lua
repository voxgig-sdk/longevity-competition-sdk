...
local RankPreviewEntity = {}
RankPreviewEntity.__index = RankPreviewEntity


function RankPreviewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RankPreviewEntity)
  self._name = "rank_preview"
  self._client = client
...
function RankPreviewEntity:get_name()
  return self._name
end


function RankPreviewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RankPreviewEntity.new(self._client, opts)
end
...
function RankPreviewEntity:mark_deleted()
  self._deleted = true
end


function RankPreviewEntity:deleted()
  return true == self._deleted
end


function RankPreviewEntity:data_set(args)
  if args ~= nil then
...
function RankPreviewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RankPreviewEntity:match_set(args)
  if args ~= nil then
...
function RankPreviewEntity:match_get()
...
function RankPreviewEntity:stream(action, args, callopts)
...
---@param reqdata RankPreviewCreateData
---@param ctrl? table
---@return RankPreview
---@return string? err
function RankPreviewEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function RankPreviewEntity:_run_op(ctx, post_done)
  local utility = self._utility
