...
local LeaderboardEntity = {}
LeaderboardEntity.__index = LeaderboardEntity


function LeaderboardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LeaderboardEntity)
  self._name = "leaderboard"
  self._client = client
...
function LeaderboardEntity:get_name()
  return self._name
end


function LeaderboardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LeaderboardEntity.new(self._client, opts)
end
...
function LeaderboardEntity:mark_deleted()
  self._deleted = true
end


function LeaderboardEntity:deleted()
  return true == self._deleted
end


function LeaderboardEntity:data_set(args)
  if args ~= nil then
...
function LeaderboardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LeaderboardEntity:match_set(args)
  if args ~= nil then
...
function LeaderboardEntity:match_get()
...
function LeaderboardEntity:stream(action, args, callopts)
...
---@param reqmatch LeaderboardListMatch
---@param ctrl? table
---@return Leaderboard[]
---@return string? err
function LeaderboardEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function LeaderboardEntity:_run_op(ctx, post_done)
  local utility = self._utility
