...
import (
	...
	omni "github.com/voxgig-sdk/longevity-competition-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LongevityCompetitionError); is {
				out := map[string]any{
					"name":    "LongevityCompetitionError",
...
func omniCtx(arg any, client *sdk.LongevityCompetitionSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LongevityCompetitionSDK); is {
			client = live
