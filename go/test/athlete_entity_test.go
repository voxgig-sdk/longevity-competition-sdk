...
func TestAthleteEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Athlete(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Athlete(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LONGEVITYCOMPETITION_TEST_ATHLETE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		athleteRef01Match := map[string]any{}

		athleteRef01ListResult, err := athleteRef01Ent.List(athleteRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, athleteRef01ListOk := athleteRef01ListResult.([]any)
		if !athleteRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", athleteRef01ListResult)
		}
...
func athleteBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LONGEVITYCOMPETITION_TEST_ATHLETE_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"LONGEVITYCOMPETITION_TEST_ATHLETE_ENTID": idmap,
		"LONGEVITYCOMPETITION_TEST_LIVE":      "FALSE",
		"LONGEVITYCOMPETITION_TEST_EXPLAIN":   "FALSE",
	})

	idmapResolved := core.ToMapAny(env["LONGEVITYCOMPETITION_TEST_ATHLETE_ENTID"])
