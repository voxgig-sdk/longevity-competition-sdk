...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("LONGEVITYCOMPETITION_TEST_LIVE") == "TRUE" ||
		os.Getenv("LONGEVITYCOMPETITION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("LONGEVITYCOMPETITION_TEST_EXPLAIN"); explain != "" {
