...
func TestRankPreviewEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LONGEVITYCOMPETITION_TEST_RANK_PREVIEW_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		rankPreviewRef01Data = core.ToMapAny(rankPreviewRef01DataResult)
...
func rank_previewBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LONGEVITYCOMPETITION_TEST_RANK_PREVIEW_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LONGEVITYCOMPETITION_TEST_RANK_PREVIEW_ENTID"])
