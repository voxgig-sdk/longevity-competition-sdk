...
const __1 = require("..");
const SDK_NAME = 'LongevityCompetitionSDK';
...
const TEST_SEED = { "entity": { "athlete": { "test01": { "id": "test01" } }, "bortz_age": { "test01": { "id": "test01" } }, "competition": { "test01": { "id": "test01" } }, "leaderboard": { "test01": { "id": "test01" } }, "pheno_age": { "test01": { "id": "test01" } }, "rank_preview": { "test01": { "id": "test01" } }, "reference": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+LongevityCompetitionSDK\b/.test(code) ||
        /\bLongevityCompetitionSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+LongevityCompetitionSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/LongevityCompetitionSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
