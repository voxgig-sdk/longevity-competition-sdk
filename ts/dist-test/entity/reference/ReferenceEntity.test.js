...
(0, node_test_1.describe)('ReferenceEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'reference.' + op, live))
                return;
        }
        ...
        let reference_ref01_data = Object.values(setup.data.existing.reference)[0];
        // LIST
        const reference_ref01_ent = client.Reference();
        const reference_ref01_match = {};
        const reference_ref01_list = (await reference_ref01_ent.list(reference_ref01_match)).map((e) => e.data());
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/reference/ReferenceTestData.json');
    ...
    let idmap = transform(['reference01', 'reference02', 'reference03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['LONGEVITY_COMPETITION_TEST_REFERENCE_ENTID'];
...
//# sourceMappingURL=ReferenceEntity.test.js.map