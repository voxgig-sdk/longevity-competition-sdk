...
class LeaderboardEntity
  ...
  def make
    opts = @_entopts.dup
    LeaderboardEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = LongevityCompetitionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Leaderboard, Hash] the current Leaderboard data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = LongevityCompetitionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Leaderboard fields)
  def match_get
  ...
  #
  # @param reqmatch [LeaderboardListMatch, Hash, nil] match filter (any subset of
  #   Leaderboard fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Leaderboard>, Array] the matching Leaderboard items; raises LongevityCompetitionError on failure
