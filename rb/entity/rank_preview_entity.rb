...
class RankPreviewEntity
  ...
  def make
    opts = @_entopts.dup
    RankPreviewEntity.new(@_client, opts)
  end
  ...
  # @return [RankPreview, Hash] the current RankPreview data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of RankPreview fields)
  def match_get
  ...
  #
  # @param reqdata [RankPreviewCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [RankPreview, Hash] the created RankPreview; raises LongevityCompetitionError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = LongevityCompetitionHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
