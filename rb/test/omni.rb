...
module LongevityCompetitionOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if LongevityCompetitionOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = LongevityCompetitionOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| LongevityCompetitionOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || LongevityCompetitionOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = LongevityCompetitionSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = LongevityCompetitionError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { LongevityCompetitionOmni.errify(err) }
