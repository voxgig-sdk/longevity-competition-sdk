...
class AthleteDirectTest < Minitest::Test
  def test_direct_list_athlete
    setup = athlete_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-athlete", setup[:live] ? "live" : "unit")
    if _should_skip
...
def athlete_direct_setup(mockres)
