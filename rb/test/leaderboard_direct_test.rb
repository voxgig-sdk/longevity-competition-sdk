...
class LeaderboardDirectTest < Minitest::Test
  def test_direct_list_leaderboard
    setup = leaderboard_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-leaderboard", setup[:live] ? "live" : "unit")
    if _should_skip
...
def leaderboard_direct_setup(mockres)
