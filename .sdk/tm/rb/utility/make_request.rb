...
module LongevityCompetitionUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LongevityCompetitionResponse.new({})
    ...
    if fetch_err
      ...
      response = LongevityCompetitionResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LongevityCompetitionResponse.new(fetched)
    else
