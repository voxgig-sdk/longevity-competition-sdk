...
module LongevityCompetitionUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = LongevityCompetitionContext.new({}, nil)
    end
    op = ctx.op || LongevityCompetitionOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || LongevityCompetitionResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(LongevityCompetitionError) ? err.msg : err.to_s
    ...
    sdk_err = LongevityCompetitionError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(LongevityCompetitionError)
