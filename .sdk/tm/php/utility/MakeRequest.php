...
class LongevityCompetitionMakeRequest
{
    public static function call(LongevityCompetitionContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new LongevityCompetitionResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new LongevityCompetitionResponse($fetched);
        } else {
