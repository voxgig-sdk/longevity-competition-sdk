...
// LongevityCompetition SDK retry feature
...
class LongevityCompetitionRetryFeature extends LongevityCompetitionBaseFeature
{
    ...
    public function init(LongevityCompetitionContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (LongevityCompetitionContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(LongevityCompetitionContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(LongevityCompetitionContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
