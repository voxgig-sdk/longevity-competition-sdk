...
// LongevityCompetition SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = LongevityCompetitionOmni::makeStructRunner(
    ...
    {
        return LongevityCompetitionOmni::objmodel(LongevityCompetitionOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return LongevityCompetitionOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return LongevityCompetitionOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(LongevityCompetitionOmni::structfix($now, Struct::undef()))
                        !== json_encode(LongevityCompetitionOmni::structfix($val, Struct::undef()))) {
