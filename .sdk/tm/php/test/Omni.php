...
//    only - a typed LongevityCompetitionContext there would make every ctx
...
final class LongevityCompetitionOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new LongevityCompetitionContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new LongevityCompetitionSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new LongevityCompetitionResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new LongevityCompetitionError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new LongevityCompetitionResponse($clean['response']);
    ...
    public static function err_from_map($m): ?LongevityCompetitionError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new LongevityCompetitionError(is_string($code) ? $code : '', $msg);
    }
