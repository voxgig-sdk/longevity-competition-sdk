...
// LongevityCompetition SDK feature test
//
...
class FtCtrl extends LongevityCompetitionControl
{
...
class FtClient
{
    ...
    private LongevityCompetitionUtility $utility;

    public function __construct(LongevityCompetitionUtility $utility, array $options)
    {
    ...
    public function get_utility(): LongevityCompetitionUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LongevityCompetitionContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LongevityCompetitionContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LongevityCompetitionContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LongevityCompetitionConfig::make_feature($name);
    ...
    private static function buildUrl(LongevityCompetitionSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LongevityCompetitionError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LongevityCompetitionResponse($fetched);
        }
    ...
    private function fail(LongevityCompetitionContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LongevityCompetitionContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
