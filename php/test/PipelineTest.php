...
// LongevityCompetition SDK pipeline test
//
...
{
    private static function utility(): LongevityCompetitionUtility
    {
        return new LongevityCompetitionUtility();
    }
    ...
    private static function ctx(array $over = []): LongevityCompetitionContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new LongevityCompetitionContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new LongevityCompetitionOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof LongevityCompetitionError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new LongevityCompetitionOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = LongevityCompetitionMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new LongevityCompetitionOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = LongevityCompetitionMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new LongevityCompetitionOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = LongevityCompetitionMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new LongevityCompetitionSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = LongevityCompetitionMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new LongevityCompetitionResponse([]),
            'result' => new LongevityCompetitionResult([]),
        ]);
        [, $err] = LongevityCompetitionMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new LongevityCompetitionSpec([]),
            'response' => null,
            'result' => new LongevityCompetitionResult([]),
        ]);
        [, $err] = LongevityCompetitionMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new LongevityCompetitionSpec(['step' => 's']),
            'response' => new LongevityCompetitionResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new LongevityCompetitionResult([]),
        ]);
        [, $err] = LongevityCompetitionMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new LongevityCompetitionSpec(['step' => 's']),
            'response' => new LongevityCompetitionResponse(self::resp(200, ['v' => 1])),
            'result' => new LongevityCompetitionResult([]),
        ]);
        [, $err] = LongevityCompetitionMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new LongevityCompetitionSpec(['step' => 's']),
            'response' => new LongevityCompetitionResponse(self::resp(200, ['v' => 2])),
            'result' => new LongevityCompetitionResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        LongevityCompetitionMakeResponse::call($ctx);
    ...
    {
        $preset = new LongevityCompetitionResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new LongevityCompetitionSpec([]),
            'response' => new LongevityCompetitionResponse([]),
            'result' => new LongevityCompetitionResult([]),
        ...
        [$got, $err] = LongevityCompetitionMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new LongevityCompetitionResult([])]);
        [, $err] = LongevityCompetitionMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new LongevityCompetitionSpec([]), 'result' => null]);
        [, $err] = LongevityCompetitionMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new LongevityCompetitionSpec(['step' => 's']),
            'result' => new LongevityCompetitionResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new LongevityCompetitionOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = LongevityCompetitionMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new LongevityCompetitionSpec(['step' => 's']),
            'result' => new LongevityCompetitionResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new LongevityCompetitionOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = LongevityCompetitionMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new LongevityCompetitionResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new LongevityCompetitionSpec([]),
            'result' => new LongevityCompetitionResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = LongevityCompetitionMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = LongevityCompetitionMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new LongevityCompetitionError('boom', 'boom');
        $utility->fetcher = function (LongevityCompetitionContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new LongevityCompetitionSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = LongevityCompetitionMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (LongevityCompetitionContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = LongevityCompetitionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(LongevityCompetitionResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        LongevityCompetitionMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (LongevityCompetitionContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new LongevityCompetitionSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = LongevityCompetitionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new LongevityCompetitionResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new LongevityCompetitionSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = LongevityCompetitionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = LongevityCompetitionMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = LongevityCompetitionMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new LongevityCompetitionResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, LongevityCompetitionDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new LongevityCompetitionResult(['ok' => false])]);
        $this->expectException(LongevityCompetitionError::class);
        LongevityCompetitionDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new LongevityCompetitionResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', LongevityCompetitionMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        LongevityCompetitionMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new LongevityCompetitionBaseFeature();
        $b = new LongevityCompetitionBaseFeature();
        LongevityCompetitionFeatureAdd::call($ctx, $a);
        LongevityCompetitionFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): LongevityCompetitionBaseFeature
    {
        $f = new LongevityCompetitionBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        LongevityCompetitionFeatureAdd::call($ctx, self::named_feature('a'));
        LongevityCompetitionFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        LongevityCompetitionFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        LongevityCompetitionFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        LongevityCompetitionFeatureAdd::call($ctx, $replace);
        ...
        LongevityCompetitionFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): LongevityCompetitionContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new LongevityCompetitionSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = LongevityCompetitionPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        LongevityCompetitionPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new LongevityCompetitionResponse(['headers' => null]),
            'result' => new LongevityCompetitionResult([]),
        ]);
        LongevityCompetitionResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        LongevityCompetitionResultBody::call($ctx);
