...
// LongevityCompetition SDK RankPreview entity
...
class RankPreviewEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new RankPreviewEntity($this->_client, $opts);
    }

    /**
     * @param RankPreview|array $args RankPreview data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = LongevityCompetitionHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return RankPreview|array The current RankPreview data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of RankPreview fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = LongevityCompetitionHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of RankPreview fields).
     */
    ...
    /**
     * Create a new RankPreview.
     *
     * @param RankPreviewCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed RankPreviewCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return RankPreview|array The created RankPreview as an assoc-array at the
     *   SDK boundary; throws LongevityCompetitionError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = LongevityCompetitionHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
