...
// LongevityCompetition SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class LongevityCompetitionTestFeature extends LongevityCompetitionBaseFeature
{
    ...
    public function init(LongevityCompetitionContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (LongevityCompetitionContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = LongevityCompetitionParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (LongevityCompetitionContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(LongevityCompetitionContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = LongevityCompetitionParam::call($ctx, $k);
